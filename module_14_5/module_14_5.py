...
async def set_username(message, state):
    registr_set = crud_functions.is_included(message.text)
    if registr_set == False:
        await state.update_data(username=message.text)
        data = await state.get_data()
        await message.answer('Введите свой email:')
        await RegistrationState.email.set()
    else:
        await message.answer(f"{message.text} уже существует, придумай что-то другое")
...
async def set_email(message, state):
    ...
    data = await state.get_data()
    await message.answer("Введите свой возраст(только цифры):")
...
async def set_age(message, state):
    try:
        ...
        await message.answer("Ошибка: попробуйте ещё раз, что то пошло не так.")
        await state.finish()
